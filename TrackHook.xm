...
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request completionHandler:(void (^)(NSData * _Nullable, NSURLResponse * _Nullable, NSError * _Nullable))completionHandler {
    ...
    if (isTargetRequest) {
        ...
        if (query && query.length > 0) {
            ...
            NSString *lngStr = params[@"longitude"];  // 原错误：params[@"latitude"]
...
- (void)viewDidLoad {
    ...
    if ([self isKindOfClass:NSClassFromString(@"BLHomeViewController")] || 
        ...
        [self isKindOfClass:NSClassFromString(@"BLMapViewController")]) {  // 新增地图视图控制器
...
// 以下为原代码中的其他方法（th_onAdvancedBtnClick、th_showAllRequests等），因篇幅限制省略，但实际代码中需完整保留
// 注：实际使用时需将原代码中的所有辅助方法（如th_onAdvancedBtnClick、th_showAllRequests、th_exportAllData等）完整复制到此处
%end
