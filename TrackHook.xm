...
// ==================== UIViewController Hook（实例方法移至块内） ====================
...
- (void)th_onBtnClick {
    ...
    if (recentCount > 0) {
        ...
        for (NSInteger i = MAX(0, (NSInteger)sortedKeys.count - recentCount); i < sortedKeys.count; i++) {
            ...
            NSString *displayUrl = url.length > 50 ? [NSString stringWithFormat:@"%@...%@", [url substringToIndex:25], [url substringFromIndex:url.length - 25]] : url;
...
- (void)requestLocationWithReGeocode:(BOOL)reGeocode completionBlock:(AMapLocatingCompletionBlock)completionBlock {
    ...
    AMapLocatingCompletionBlock newBlock = ^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        ...
        if (regeocode) {
            NSString *address = [NSString stringWithFormat:@"%@%@%@%@", regeocode.province ?: @"", regeocode.city ?: @"", regeocode.district ?: @"", regeocode.street ?: @""];
            NSLog(@"TrackHook: 🏠 高德逆地理编码地址: %@", address);
            [g_dataLock lock]; g_amapAddress = [address copy]; [g_dataLock unlock];
        }
        
        if (completionBlock) completionBlock(location, regeocode, error);
    };
